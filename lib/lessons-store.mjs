// The Self-Improvement Oracle's memory store — an append-only journal that
...
export function appendLesson(root, { text, kind = 'decision', paths = [], source = 'correction', learned = null }) {
  ...
  if (hit) {
    ...
    lessons.push({ text: text.trim(), pattern: text.trim(), kind, paths, source, learned, count: 1, confidence: source === 'correction' ? 0.9 : 0.4 })
  }
