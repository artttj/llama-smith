...
export function buildContext(root, files, patterns) {
...
const ARCH_RELEVANT = [
  /readme/i, /^package\.json$/, /^composer\.json$/, /^pyproject\.toml$/, /^go\.mod$/, /^Cargo\.toml$/,
...
- datamodel: the core data entities or domain models and how they relate
...
export async function scanArchitecture(root, { local = false, host } = {}) {
  ...
  const prompt = `${ARCH_PROMPT}${ARCH_OUTPUT}\n\n--- REPO (${basename(root)}) ---\n${buildContext(root, files, ARCH_RELEVANT)}`
