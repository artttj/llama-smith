...
// Assemble one lane's full prompt: lane brief + output rule + repo context, plus a
// re-check block for findings the swarm missed on prior runs (recorded via --missed
// and threaded in as missedHints by the pipeline).
...
export const SCAN_PATTERNS = [...RELEVANT.deploy, ...RELEVANT.secret, ...RELEVANT.cron]
...
Report concise, factual map items. Describe capabilities and responsibilities ("X handles the OAuth token lifecycle"), never file trees. REQUIRED: provide an overview. Cover, only where the files support it:
- overview: REQUIRED. What the app is, what problem it solves, and its core purpose. Cite README, package.json description, or main entry file.
...
Each item MUST cite ONE concrete file that demonstrates it. Do not invent; omit optional items if unsupported, but NEVER omit overview.`
...
// One swarm pass over a pre-built per-lane context. The three lanes dispatch
// concurrently. Returns merged findings + lane telemetry. Both the repo-wide
// scan and the PR-scoped scan drive the swarm through here.
export async function scanContext(root, ctx, { local = false, host, missedHints = [] } = {}) {
  const m = models(local)
  const label = `(${basename(root)})\n`
  const mk = lane => lanePrompt(lane, label + ctx[lane], missedHints)
...
export async function scanRepo(root, { local = false, host, missedHints = [] } = {}) {
  ...
  return scanContext(root, ctx, { local, host, missedHints })
}
