...
export function parseCommits(log) {
  ...
  for (const line of log.split('\n')) {
    if (line.startsWith('@@@')) {
      const [author, email = ''] = line.slice(3).split('|')
      cur = { author: author.trim(), email: email.trim(), files: [] }
      commits.push(cur)
      continue
    }
    const f = line.trim()
...
// GitHub noreply commit emails carry the username, which gives us a real
// profile avatar at github.com/<login>.png.
const GH_NOREPLY = /^(?:\d+\+)?([^@]+)@users\.noreply\.github\.com$/i
// Commit emails are untrusted; only return a real GitHub username (its avatar
// URL is built from this), never an arbitrary string.
const VALID_LOGIN = /^[a-z\d](?:[a-z\d]|-(?=[a-z\d])){0,38}$/i
export const githubLogin = emails => {
  for (const e of emails) { const m = (e || '').match(GH_NOREPLY); if (m && VALID_LOGIN.test(m[1])) return m[1] }
  return null
}
...
const topOwner = counts => { let owner = '', top = 0; for (const [n, c] of counts) if (c > top) { top = c; owner = n }; return { owner, top } }

function ownership(fileAuthors) {
  const rows = []
  for (const [file, authors] of Object.entries(fileAuthors)) {
    if (!isCodeFile(file)) continue
    const counts = tally(authors)
    const { owner, top } = topOwner(counts)
...
function moduleOwnership(fileAuthors, { limit = 8 } = {}) {
  ...
  return [...mods.values()].map(m => {
    const { owner, top } = topOwner(m.counts)
...
function contributorCounts(commits) {
  ...
  for (const c of commits) {
    ...
    const e = by.get(c.author) || { commits: 0, emails: new Set() }
    e.commits++
    if (c.email) e.emails.add(c.email)
    by.set(c.author, e)
  }
  const top = [...by.entries()].sort((a, b) => b[1].commits - a[1].commits).slice(0, 8)
    .map(([name, v]) => ({ name, commits: v.commits, login: githubLogin(v.emails) }))
  return { total: by.size, top }
}
...
export function computeForensics(log, { limit = 12 } = {}) {
  ...
  const fileAuthors = {}
  for (const c of commits) for (const f of c.files) (fileAuthors[f] ||= []).push(c.author)
