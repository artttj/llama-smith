...
export function repoFreshness(root, { now = new Date() } = {}) {
  ...
  let findings
  try { findings = JSON.parse(readFileSync(p, 'utf8')) } catch { return null }
  const { scannedAt } = findings
  // sha is read from the scanned repo's .smith/findings.json (untrusted) and passed
  // to git as its own argv token. Reject anything that is not a 40-char hex SHA so a
  // value like '--output=/path' cannot reach git as an option.
  const sha = /^[0-9a-f]{40}$/i.test(findings.sha) ? findings.sha : null
