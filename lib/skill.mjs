...
// Finding text, git author names and package.json script values are LLM/repo
// derived (untrusted) and get embedded into markdown the agent later trusts.
// Collapse newlines so a crafted value cannot open a new heading or list item
// and inject instructions into the forged skill.
const inline = s => String(s ?? '').replace(/[\r\n]+/g, ' ').trim()

const findingLines = list => list.map(f => `- **${sevTag(f.severity)}** — ${inline(f.text)}${cite(f.file)}`).join('\n')
const ownerLines = rows => rows.map(o => `- \`${o.file}\` — ${inline(o.owner)} (${pct(o.share)}%, ${o.contributors} contributor${o.contributors > 1 ? 's' : ''})`).join('\n')
const moduleLines = mods => mods.map(m => `- \`${m.module}/\` — ${inline(m.owner)} (${pct(m.share)}%, ${m.contributors} contributor${m.contributors > 1 ? 's' : ''})`).join('\n')
const hotspotLines = (hot, soSet) => hot.map(h => `- \`${h.file}\` — ${h.edits} edits${soSet.has(h.file) ? ' · **single-owner — highest risk**' : ''}`).join('\n')
const peopleLine = people => people.map(p => `${inline(p.name)} (${p.files} files)`).join(', ')
const couplingLines = pairs => pairs.map(c => `- \`${c.a}\` ↔ \`${c.b}\` — changed together ${c.count}×`).join('\n')
const contributorLine = fr => `${fr.contributors} contributors over the last year.${fr.topContributors?.length ? ' Most active: ' + fr.topContributors.map(t => `${inline(t.name)} (${t.commits})`).join(', ') + '.' : ''}`

const commandKind = c => c.kind || classifyCommand(`${c.cmd} ${c.raw || ''}`)
const commandLines = cmds => cmds.map(c => `- \`${inline(c.cmd)}\`${c.raw ? `  — ${inline(c.raw)}` : ''}${cite(c.file)}`).join('\n')

const groupBy = (items, groups, keyOf) => groups
  .map(([key, title]) => ({ title, items: items.filter(i => keyOf(i) === key) }))
  .filter(g => g.items.length)

const archSections = (arch, level) => groupBy(arch, ARCH_TITLES, a => a.area).map(g => block(level, g.title, g.items.map(a => `- ${inline(a.claim)}${cite(a.file)}`).join('\n'))).join('\n\n')
...
function memoryFile(full, lessons) {
  ...
  const active = graduated.length
    ...
    : `_No graduated memory yet for ${full}. The Self-Learning Memory fills this as you work and correct the agent._`
  return doc(
    `# Memory — ${full}`,
    '_Kept by the **Self-Learning Memory**._',
...
function mostNeeded(ops, hasArch, archCount) {
  ...
  if (high.length) {
    const grouped = []
    for (const f of high) { const e = grouped.find(g => g.text === f.text); if (e) { if (f.file) e.files.push(f.file) } else grouped.push({ text: f.text, files: f.file ? [f.file] : [] }) }
    parts.push(`Then mind **${high.length} operational risk${high.length > 1 ? 's' : ''}** the scan flagged:`)
    parts.push(grouped.slice(0, 3).map(g => `- ${inline(g.text)}${g.files.length ? ` _(\`${g.files[0]}\`${g.files.length > 1 ? ` +${g.files.length - 1} more` : ''})_` : ''}`).join('\n'))
  } else if (ops.length) {
...
function skillMain(skill, full, stack, scannedAt, ops, hot, refs, entrypoints, arch) {
  ...
  return doc(
    ...
    block(2, 'Memory', 'This skill learns. The **Self-Learning Memory** keeps long-term memory in [`memory.md`](memory.md) — corrections and recurring session lessons, applied automatically on the next run.'),
  )
}
...
export function buildSkillFiles(r, { name, lessons = [], scannedAt = 'this run', stack = '', commands = [], entrypoints = [], boundaries = [], architecture = [], forensics = null } = {}) {
  ...
  const soSet = new Set((forensics?.singleOwner || []).map(s => s.file))

  const files = [], refs = []
  const addRef = (file, title, body) => { refs.push({ file, title }); files.push({ path: file, body }) }

  if (architecture.length) addRef('references/architecture.md', 'Architecture & application map', architectureFile(full, architecture))
  ...
  if (forensics?.busFactor) addRef('references/forensics.md', 'Knowledge risk — bus factor & owners', forensicsFile(full, forensics))
  addRef('references/commands.md', 'Real commands', commandsFile(full, commands))
  if (boundaries.length) addRef('references/boundaries.md', 'Do-not-touch boundaries', boundariesFile(full, boundaries))

  files.push({ path: 'memory.md', body: memoryFile(full, lessons) })
  files.unshift({ path: 'SKILL.md', body: skillMain(skill, full, stack, scannedAt, ops, hot, refs, entrypoints, architecture) })
  files.push({ path: 'AGENTS.md', body: agentsFile(full, { stack, scannedAt, eps: entrypoints, arch: architecture, ops, hot, cmds: commands, bounds: boundaries, fr: forensics, soSet }) })
