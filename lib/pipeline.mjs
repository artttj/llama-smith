...
export async function runPipeline(root, opts = {}) {
  ...
  const findings = { project, sha: headSha(root), scannedAt, stack, architecture, opsFindings: supported, hotspots, commands, entrypoints, boundaries, droppedByOracle: dropped.length }
  ...
  if (!opts.scanOnly) {
    ...
    const built = buildSkillFiles(
      ...
      { name: `${project}-smith`, lessons, scannedAt: scannedAt.slice(0, 10), stack, architecture, commands, entrypoints, boundaries }
    )
